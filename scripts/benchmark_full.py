...
def operating_threshold(y: np.ndarray, p: np.ndarray, target_recall: float = .8) -> float:
    precision, recall, thresholds = precision_recall_curve(y, p)
    eligible = np.flatnonzero(recall[:-1] >= target_recall)
    if len(eligible):
        best = eligible[int(np.argmax(precision[:-1][eligible]))]
    else:
        best = int(np.argmax(2 * precision[:-1] * recall[:-1] / np.maximum(precision[:-1] + recall[:-1], 1e-12)))
    return float(thresholds[best])


def scores(y: np.ndarray, p: np.ndarray, threshold: float) -> dict[str, float]:
    pred = (p >= threshold).astype(int)
    return {"precision": float(precision_score(y, pred, zero_division=0)), "recall": float(recall_score(y, pred, zero_division=0)), "f1": float(f1_score(y, pred, zero_division=0)), "roc_auc": float(roc_auc_score(y, p)), "pr_auc": float(average_precision_score(y, p)), "threshold": threshold}
...
def main() -> None:
    ...
    n = len(x)
    train_end, val_end = int(n * .7), int(n * .85)
    x_train, x_val, x_test = x.iloc[:train_end], x.iloc[train_end:val_end], x.iloc[val_end:]
    y_train, y_val, y_test = y[:train_end], y[train_end:val_end], y[val_end:]
    weights = np.where(y_train == 1, (y_train == 0).sum() / max((y_train == 1).sum(), 1), 1.0)
    model = HistGradientBoostingClassifier(max_iter=150, learning_rate=.08, max_leaf_nodes=31, l2_regularization=1.0, random_state=42)
    model.fit(x_train, y_train, sample_weight=weights)
    threshold = operating_threshold(y_val, model.predict_proba(x_val)[:, 1])
    result = {"dataset": "kaggle", "rows": n, "features": int(x.shape[1]), "split": "chronological 70/15/15", "fraud_rate": float(y.mean()), "selected_model": scores(y_test, model.predict_proba(x_test)[:, 1], threshold), "selected_model_at_065": scores(y_test, model.predict_proba(x_test)[:, 1], .65), "target_recall": .8}
