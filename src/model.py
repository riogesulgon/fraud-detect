...
def generate_data(n: int = 1000, seed: int = 42) -> tuple[list[dict], np.ndarray]:
    ...
    rows = []
    for _ in range(n):
        row = {"transaction_amount": round(float(rng.lognormal(4.2, 1.0)), 2), "merchant_category": str(rng.choice(categories)), "country_risk_score": round(float(rng.random()), 4), "account_age_days": int(rng.integers(1, 2500)), "transaction_count_24h": int(rng.poisson(4)), "failed_auth_attempts_24h": int(rng.poisson(0.4)), "is_new_device": bool(rng.integers(0, 2)), "hour_utc": int(rng.integers(0, 24))}
        rows.append(row)
    scores = np.array([r["country_risk_score"] * 1.8 + r["is_new_device"] * 1.2 + r["failed_auth_attempts_24h"] * .7 + r["transaction_count_24h"] * .08 + (r["transaction_amount"] > 250) * .7 + (r["account_age_days"] < 30) * .8 for r in rows])
